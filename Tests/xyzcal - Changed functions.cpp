...
#define XYZCAL_PINDA_HYST_MIN 16 //20  //50um ZRIB Changes
#define XYZCAL_PINDA_HYST_MAX 80 //100 //250um ZRIB Changes
#define XYZCAL_PINDA_HYST_DIF 4 //5   //12.5um ZRIB Changes
...
//		int8_t _pinda = _PINDA;
		xyzcal_lineXYZ_to((r&1)?(cx+1024):(cx-1024), cy - 1024 + r*64, z, 2*delay_us, 0); // 80 steps per unit
		xyzcal_lineXYZ_to(_X, _Y, min_z, delay_us, 1);
		xyzcal_lineXYZ_to(_X, _Y, max_z, delay_us, -1);
		z = (int16_t)count_position[2];
...
#define MAX_DIAMETR 480 //600 //Max diameter of center find ZRIB_mods
...
const int16_t xyzcal_point_xcoords[4] PROGMEM = { 1200, 22000, 22000, 1200 }; //points to 80 steps per unit 
const int16_t xyzcal_point_ycoords[4] PROGMEM = {600, 600, 19800, 19800}; //points to 80 steps per unit
...
//	int16_t dz = 100;
	...
	{
		uint16_t ad = 0;
		if (xyzcal_spiral8(x0, y0, z, 100, 900, 320, 1, &ad)) //dz=100 radius=900 delay=400 //reduce everything to work with 80
		{
			int16_t x_on = _X;
			int16_t y_on = _Y;
			int16_t z_on = _Z;
