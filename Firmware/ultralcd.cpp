        ...
        if (lcd_commands_step == 9 && !blocks_queued() && cmd_buffer_empty())
        {
            ...
            } else
            {
                ...
                //enquecommand_P(PSTR("G1 X100.0 E12.5 F1000.0")); //intro line
					...
					enquecommand_P(PSTR("G1 X60.0 E7.0 F614.0")); //intro line
					enquecommand_P(PSTR("G1 X120.0 E10.0 F614.0")); //intro line
				#else
					enquecommand_P(PSTR("G1 X60.0 E9.0 F1000.0")); //intro line
					enquecommand_P(PSTR("G1 X100.0 E12.5 F1000.0")); //intro line
				#endif // FILAMENT_3MM
...
#ifdef FILAMENT_3MM /*RAMPS*/
			enquecommand_P(PSTR("G1 X50 Y155"));
			enquecommand_P(PSTR("G1 Z0.150 F4421.000"));
			enquecommand_P(PSTR("G1 F1080"));
			enquecommand_P(PSTR("G1 X75 Y155 E1.5351"));
			enquecommand_P(PSTR("G1 X100 Y155 E1.2281"));
			enquecommand_P(PSTR("G1 X200 Y155 E1.61352"));
			enquecommand_P(PSTR("G1 X200 Y135 E0.40633"));
			enquecommand_P(PSTR("G1 X50 Y135 E2.2276"));
			enquecommand_P(PSTR("G1 X50 Y115 E0.30325"));
			enquecommand_P(PSTR("G1 X200 Y115 E2.2276"));
			enquecommand_P(PSTR("G1 X200 Y95 E0.30325"));
			enquecommand_P(PSTR("G1 X50 Y95 E2.2276"));
			enquecommand_P(PSTR("G1 X50 Y75 E0.30325"));
			enquecommand_P(PSTR("G1 X200 Y75 E2.2276"));
			enquecommand_P(PSTR("G1 X200 Y55 E0.30325"));
			enquecommand_P(PSTR("G1 X50 Y55 E2.2276"));
#else
			enquecommand_P(PSTR("G1 X50 Y155"));
			enquecommand_P(PSTR("G1 Z0.150 F7200.000"));
			enquecommand_P(PSTR("G1 F1080"));
...
static float count_e(float layer_heigth, float extrusion_width, float extrusion_length) {
	...
	#ifdef FILAMENT_3MM
		float extr = extrusion_length * layer_heigth * extrusion_width / (M_PI * pow(2.85, 2) / 4);
	#else
		float extr = extrusion_length * layer_heigth * extrusion_width / (M_PI * pow(1.75, 2) / 4);
	#endif //FILAMENT_3MM
	/*RAMPS*/
...
bool lcd_selftest()
{
	...
	bool _result = true;
	/*RAMPS*/
	//gcode_G28(true, true, true); //added for stability, or else it fails (Loose pulley error!)
	/*RAMPS*/
