...
//#define E0_STEP_PIN  34 //PC3 (+)
	...
	//#define Z_DIR_PIN    36 //D48 = PL1	0 (-) // moded to + for top motors
...
/*RAMPS*/
	...
	case 2: if (dir) PORTL |= 2; else PORTL &= ~2; break;		//1 - //moded to +
...
/*RAMPS*/
	...
	if (portA & 64) dir_bits |= 8;	//6 +  //3DP_Mods for Z motors on the upper side
	dir_bits ^= 0x03; //invert XY, do not invert ZE
#endif
	return dir_bits;
}
...
/*RAMPS*/
	...
	dir_bits ^= 0x03; //invert XYZ, do not invert E //3DP_Mods for Z motors on the upper side 
...
uint16_t sm4_line_xyze_ui(uint16_t da, uint16_t db, uint16_t dz, uint16_t de)  //Crazy attemp to move at 80 (uint16_t dx, uint16_t dy, uint16_t dz, uint16_t de)

{
  uint16_t dx = da * .8; //this is crazy shit
  uint16_t dy = db * .8;
